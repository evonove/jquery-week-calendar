...
test("EventHeader should use 24 hour format", function() {
  expect(2);

  var $calendar = $('#calendar'),
      eventHeaderFunc = $calendar.weekCalendar('option', 'eventHeader'),
      _privateWeekCalendar = $calendar.data('ui-weekCalendar'),
      _event = {
        'id': 1,
        'start': new Date('2013-06-28T09:00:00+02:00'),
        'end': new Date('2013-06-28T18:00:00+02:00'),
        'title': 'Coding week calendar'
      };
  ok($.isFunction(eventHeaderFunc), 'check eventHeader is a function');
  equal(eventHeaderFunc(_event, $calendar), '09:00 to 18:00');
});

test('issue #49: wrong calculation of DateLastMilliOfWeek (Test date Last|First Day|Milli OfWeek())', function() {
  var $calendar = $('#calendar');
  $calendar.weekCalendar({
